...
(function () {
  ...
  function detectType(u) {
    try {
      ...
      var p = url.pathname
      if (/v\.douyin\.com$/.test(host)) return { type: '短链接', sub: '提交后解析', key: 'short' }
      if (/^\/(share\/)?video\//.test(p)) return { type: '视频', sub: '', key: 'video' }
      if (/^\/(share\/)?note\//.test(p)) return { type: '图文', sub: '', key: 'gallery' }
      if (/^\/user\//.test(p)) return { type: '用户主页', sub: '', key: 'user' }
      if (/collection/.test(p)) return { type: '合集', sub: '', key: 'mix' }
      if (/^\/(share\/)?music\//.test(p)) return { type: '音乐', sub: '', key: 'music' }
      if (/live/.test(p)) return { type: '直播', sub: '仅识别，不保证可下载', key: 'live' }
  ...
  function seedHistory(now) {
    ...
    for (var i = 0; i < 58; i++) {
      ...
      items.push({
        ...
        file_path: 'Downloaded/' + a.name + '/' + (isGallery ? 'post' : 'post') + '/' +
  ...
  var state = {
    ...
    hist: { page: 1, size: 50, search: '', type: 'all', from: '', to: '', sort: 'download_time', job: '' },
    timer: null
  }
  ...
  function applyTheme(theme) {
    ...
    try { localStorage.setItem('webui2-theme', theme) } catch (e) {}
  }
  ...
  ;(function initTheme() {
    var saved = null
    try { saved = localStorage.getItem('webui2-theme') } catch (e) {}
  ...
  function renderChrome() {
    ...
    if (state.devState !== 'offline' && state.health === 'ok') {
      ...
      netTime.textContent = fmtClock(state.lastNetOk) ? ' · 最后联通 ' + fmtClock(state.lastNetOk) : ''
    } else {
  ...
  function renderDl() {
    ...
    $('#dlSubmit').disabled = submitCount === 0 || state.submitting || state.devState === 'offline'
    renderNotice(parsed)
  }
  ...
  function jobCard(job) {
    ...
    if (isLive(job)) {
      ...
      cancel.disabled = !!job._cancelling
  ...
  function renderBanners() {
    ...
    var net = $('#netBanner')
    if (state.devState === 'error' || state.devState === 'offline') {
      net.hidden = false
      $('#netBannerTime').textContent = '最后成功 ' + fmtClock(state.lastNetOk)
    } else {
      net.hidden = true
    }
  }
  ...
  function histFiltered() {
    ...
    if (h.search) {
      ...
      items = items.filter(function (it) {
        return it.author_name.toLowerCase().indexOf(q) !== -1 ||
  ...
  $('#histReset').addEventListener('click', function () {
    state.hist = { page: 1, size: state.hist.size, search: '', type: 'all', from: '', to: '', sort: 'download_time', job: '' }
    syncHistForm()
    renderHistory()
  })
  ...
  function renderSettings() {
    ...
    if (state.health === 'ok' && state.devState !== 'offline') {
  ...
  $('#devReset').addEventListener('click', function () {
    state.jobs = seedJobs(Date.now())
    state.history = seedHistory(Date.now())
    state.hist = { page: 1, size: 50, search: '', type: 'all', from: '', to: '', sort: 'download_time', job: '' }
    state.submitting = false
    state.keptUrls = []
    dlInput.value = ''
    setDevState('normal')
