...
| GET | `/api/v1/downloads` | Paginated download history (filters: `page`/`size`/`author`/`title`/`aweme_type`/`job_id`/`date_from`/`date_to`) |
| GET | `/api/v1/downloads/authors` | Top downloaded authors in the last N days (`days`/`limit` params) |
